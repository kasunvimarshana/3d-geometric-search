 ...
 * Implements IModelLoader for glTF and GLB formats
 */
export class GLTFModelLoader implements IModelLoader {
  ...
  async load(data: ArrayBuffer | string, filename: string): Promise<Model> {
    return new Promise((resolve, reject) => {
      const loadCallback = (gltf: unknown): void => {
        try {
          const model = this.parseGLTF(
            gltf as { scene: THREE.Group },
            filename
          );
          resolve(model);
        } catch (error) {
          reject(new Error(`Failed to parse glTF model: ${error}`));
        }
      };

      const errorCallback = (error: unknown): void => {
        const message = error instanceof Error ? error.message : String(error);
        reject(new Error(`Failed to load glTF model: ${message}`));
      };

      if (typeof data === "string") {
        this.loader.load(data, loadCallback, undefined, errorCallback);
      } else {
  ...
  private parseGLTF(gltf: { scene: THREE.Group }, filename: string): Model {
    ...
    const processNode = (node: THREE.Object3D, parentId?: string): string => {
      ...
      const section: ModelSection = {
        ...
        boundingBox,
        metadata: { userData: node.userData },
        visible: true,
        selected: false,
        highlighted: false,
      };
    ...
    return {
      id: `model_${Date.now()}`,
      name: filename,
      format: filename.endsWith(".glb") ? ModelFormat.GLB : ModelFormat.GLTF,
      sections,
      geometries,
      materials,
      rootSectionIds,
