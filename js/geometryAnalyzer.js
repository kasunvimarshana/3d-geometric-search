 ...
 * Provides comprehensive geometric analysis and similarity comparison
 *
...
export class GeometryAnalyzer {
  ...
  /**
   * Analyze a Three.js geometry and extract comprehensive features
   *
   * Features extracted:
   * - Basic: vertex count, face count
   * - Spatial: bounding box, center of mass
   * - Shape: volume, surface area, compactness, aspect ratio
   *
   * @param {THREE.BufferGeometry} geometry - The geometry to analyze
   * @param {string} name - Unique identifier for this model
   * @returns {Object|null} Features object or null if analysis fails
   */
  analyzeGeometry(geometry, name) {
    ...
    if (features.volume > 0 && features.surfaceArea > 0) {
      features.compactness =
        Math.pow(features.volume, 2 / 3) / features.surfaceArea;
    }
  ...
  calculateSimilarity(features1, features2) {
    ...
    // Compare compactness
    const compactnessDiff = Math.abs(
      features1.compactness - features2.compactness
    );
    const maxCompactness = Math.max(
      features1.compactness,
      features2.compactness
    );
    const compactnessSimilarity =
      maxCompactness > 0 ? 1 - compactnessDiff / maxCompactness : 1;
